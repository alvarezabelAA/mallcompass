...
module.exports = (app) => {
  ...
  /* REGISTRO DE TIENDA*/
  app.options('/tiendas/relCC', cors());
  app.post('/tiendas/relCC', cors(),(req, res)=>{
    console.log("ejecucion metodo POST");
    let query = `SELECT * FROM logintokens WHERE token = '${req.query.token}'`;
    conn.query(query, (error, filas) => {
      if(error){
        ...
        if(filas.length == 0){
          ...
          console.log("encontro el token");
          /*INSERT EN TABLA TIENDA*/
          let query = `INSERT INTO rel_cc_tiendas(id_centroComercial, id_tienda) VALUES (?,?)`;
          const { idComercial, id_tienda } = req.body;
          const values = [idComercial, id_tienda];
          conn.query(query, values, (error, filas) => {
            if(error){
              res.json({ status: 0, mensaje: "error en DB", datos:error });
            }else{
